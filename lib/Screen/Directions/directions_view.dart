...
class _DirectionsViewState extends State<DirectionsView> {
  ...
  addMakers() {
    ...
    final Marker marker = Marker(
      ...
      position: LatLng(widget?.placeBloc?.formLocation?.lat,
          widget?.placeBloc?.formLocation?.lng),
      infoWindow: InfoWindow(
          title: widget?.placeBloc?.formLocation?.name,
          snippet: widget?.placeBloc?.formLocation?.formattedAddress),
      icon: checkPlatform
  ...
  void getRouter() async {
    ...
    polyLines.clear();
    var router;
    LatLng _fromLocation = LatLng(widget?.placeBloc?.formLocation?.lat,
        widget?.placeBloc?.formLocation?.lng);
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Text(
              ...
              style: TextStyle(
                  fontSize: 18, color: greyColor, fontWeight: FontWeight.bold),
            ),
          ],
        ));
  }
